...
class ReportTask(QgsTask):
    def __init__(self, params, bridge):
        super().__init__('Generazione Report PSI', QgsTask.CanCancel)
        self.params = params
        self.bridge = bridge
        self.results = None
        self.error_msg = None
    ...
    def _accelerazione(feats, campi_d, coerente_mask):
        ...
        if not campi_d or coerente_mask is None:
            QgsMessageLog.logMessage(
                ...
                "maschera di coerenza mancante).", "InSAR Report", Qgis.Info)
            return vuoto
    ...
    def run(self):
        try:
            ...
            area_mq = da.convertAreaMeasurement(area_mq_raw, QgsUnitTypes.AreaSquareMeters)
