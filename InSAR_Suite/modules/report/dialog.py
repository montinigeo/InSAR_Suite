...
class ReportDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.cb_area_layer = _layer_combo(QgsMapLayerProxyModel.Filter.PolygonLayer)
        ...
        self.cb_ps_asc = _layer_combo(QgsMapLayerProxyModel.Filter.PointLayer)
        self.cb_ps_asc_field = QgsFieldComboBox()
        self.cb_ps_asc_field.setFilters(QgsFieldProxyModel.Filter.Numeric)
        ...
        self.cb_ps_desc = _layer_combo(QgsMapLayerProxyModel.Filter.PointLayer)
        self.cb_ps_desc_field = QgsFieldComboBox()
        self.cb_ps_desc_field.setFilters(QgsFieldProxyModel.Filter.Numeric)
        ...
        self.cb_vis_asc = _layer_combo(QgsMapLayerProxyModel.Filter.PointLayer, allow_empty=True)
        self.cb_vis_desc = _layer_combo(QgsMapLayerProxyModel.Filter.PointLayer, allow_empty=True)
        ...
        self.sp_accel.setRange(1.0, 10.0)
        self.sp_accel.setSingleStep(0.1)
        self.sp_accel.setValue(1.5)
        self.sp_accel.setDecimals(2)
        self.sp_accel.setToolTip(
            "Rapporto tra la velocità dell'ultimo segmento e quella del "
            "segmento precedente (analisi piecewise, max 3 segmenti) oltre "
            "il quale segnalare un'accelerazione.")
        form_par.addRow('Soglia accelerazione:', self.sp_accel)
        ...
        # ── Pulsanti ─────────────────────────────────────────────────────────
        btn_row = QHBoxLayout()
        self.btn_run = QPushButton('Genera report')
    ...
    def _set_area_from_layer(self, lyr, source_label):
        if not lyr.isValid() or lyr.geometryType() != QgsWkbTypes.GeometryType.PolygonGeometry:
    ...
    def _update_area_status(self):
        ...
        area_mq = da.convertAreaMeasurement(area_raw, QgsUnitTypes.AreaUnit.AreaSquareMeters)
        self.lbl_area_status.setText(
    ...
    def _on_run(self):
        ...
        params = {
            ...
            'soglia_accel': self.sp_accel.value(),
        }
        ...
        self.task = ReportTask(params, self.bridge)
        QgsApplication.taskManager().addTask(self.task)

    def _on_finished(self, results):
        self.progress.setVisible(False)
        self.btn_run.setEnabled(True)
        self._log_msg('✅ Report completato.')
        dlg = ReportResultsDialog(results, parent=self)
        dlg.show()
